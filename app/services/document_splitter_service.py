...
class DocumentSplitterService:
    ...
    def __init__(self):
        ...
        # 记录日志
        logger.info(
            f"文档分割服务初始化完成，chunk_size={self.chunk_size}, secondary_chunk_size={self.chunk_size * 2}, chunk_overlap={self.chunk_overlap}"
        )
    ...
    def split_markdown(self, content: str, file_path: str = "") -> list[Document]:
        ...
        try:
            ...
            # 第三阶段：合并太小的分片（< 300字符）
            final_docs = self.merge_small_chunks(docs_after_split, min_size=300)
    ...
    def split_text(self, content: str, file_path: str = "") -> list[Document]:
        ...
        try:
            ...
            # 记录日志
            logger.info(f"文本文档分割完成: {file_path} -> {len(docs)} 个分片")
            return docs

        except Exception as e:
            logger.error(f"文本文档分割失败: {file_path}, 错误: {e}")
            raise

    def split_documents(self, content: str, file_path: str = "") -> list[Document]:
        """
        智能分割文档 (根据文件类型选择分割器)
        Args:
            content: 文档内容
    ...
    def merge_small_chunks( self, documents: List[Document], min_size: int = 300) -> list[Document]:
        """
        合并太小的分片
        Args:
            docs: 文档分片列表
