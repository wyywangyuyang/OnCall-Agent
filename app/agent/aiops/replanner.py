...
async def replanner(state: PlanExecuteState) -> Dict[str, Any]:
    """
       ...
       3. respond - 生成最终响应
    """
    logger.info("============ Replanner 节点开始重新规划计划 ============")
    ...
    if len(past_steps) >= MAX_STEPS:
        logger.info(f"已执行{len(past_steps)}步，超过最大限制{MAX_STEPS}，强制生成最终响应")
        llm = ChatQwen(
        ...
        try:
            ...
            if action == "respond":
                ...
                if new_steps:
                    ...
                    logger.info("replan但未提供新步骤，继续执行原计划")
                    return {}
...
async def _generate_response(state: PlanExecuteState, llm: ChatQwen) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(response_obj, Response):
            ...
            # 如果返回的是字典, 则取字典中的response字段
            final_response = response_obj.get("response", "") # type: ignore

        logger.info(f"最终响应生成完成，长度为：{final_response}")
        ...
        {input_text}
        ## 执行的步骤
        {format_simple_steps(past_steps)}
        ## 说明
        由于系统异常，无法生成完整响应。以上是已收集到的信息。
        """
        return {"response": fallback_response}

def format_simple_steps(past_steps: list) -> str:
    """
    格式化步骤列表（简版）
    :param past_steps:  步骤列表
    :return:
    """
    if not past_steps:
        return "无"
