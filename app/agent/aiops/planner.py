...
class Plan(BaseModel):
    ...
    计划的输出格式
    """
    steps: List[str] = Field(description="完成任务所需的不同步骤。这些步骤应该按照顺序执行，每一步都建立在前一步的基础上。")
...
async def planner(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    2. 基于经验文档和可用工具制定执行计划
    """
    logger.info("============ Planner 节点开始制定执行计划 ============")

    inpt_text = state.get("input")
    logger.info(f"用户输入：{inpt_text}")

    try:
        # 步骤1：查询内部文档，获取相关经验和最佳实践
        logger.info("查询内部文档，获取相关经验和最佳实践。。。")
        experience_docs =""
        try:
            # retrieve_knowledge 使用 response_format="content_and_artifact"
            # ainvoke() 只返回 content（字符串），不是元组
            context_str = await retrieve_knowledge.ainvoke({"query": inpt_text})
            if context_str and context_str.strip():
                experience_docs = context_str
                logger.info(f"找到相关经验文档，长度为：{len(experience_docs)}")
            else:
